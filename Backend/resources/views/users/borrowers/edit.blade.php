...
@section('page_title', 'Borrowers Edit')
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Borrowers Edit</h4>
                        <form class="forms-sample material-form" id="userForm">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <input type="text" name="city" required="required" value="{{@$user->borrowerDetails->city}}"/>
                                        <label for="input" class="control-label">City</label><i class="bar"></i>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" name="pincode" required="required" minlength="6" maxlength="6" value="{{@$user->borrowerDetails->pincode}}"/>
                                        <label for="input" class="control-label">Pincode</label><i class="bar"></i>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" name="location" required="required" value="{{@$user->borrowerDetails->location}}"/>
                                        <label for="input" class="control-label">Location</label><i class="bar"></i>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" name="phone_number" required="required" maxlength="10" value="{{@$user->borrowerDetails->phone_number}}"/>
                                        <label for="input" class="control-label">Phone Number</label><i class="bar"></i>
                                    </div>
                                    <div class="form-group">
                                        <textarea type="text" name="address" required="required" >{{@$user->borrowerDetails->address}}</textarea>
                            ...
                            <div class="button-container">
                                ...
                                <a href="{{route('usersListing')}}" class="button btn btn-danger" id="backBtn">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        $(document).ready(function() {
            $('#saveBtn').on('click', function(e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('user.update') }}", // Corrected the URL here
                    type: 'POST',
                    dataType: 'json',
                    success: function(data) {
                        if (data.status === true) {
                            ...
                            Swal.fire({
                                ...
                                window.location.href = "{{ route('usersListing') }}";
                            });
                        } else {
